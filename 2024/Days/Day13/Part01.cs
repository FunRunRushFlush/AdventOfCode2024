...
namespace Year_2024.Days.Day13;
public class Part01 : IPart
{
    ...
    //private  (int APresses, int BPresses) CalculateButtonPresses(ClawInstruction ins)
    //{
    ...
    private (bool IsPos, int EdgeCase) CheckIfPriceIsPossible(ClawInstruction ins)
    {

        var priceAngle = Math.Atan(ins.Price.Y / (double)ins.Price.X);
        var aAngle = Math.Atan(ins.A.Y / (double)ins.A.X);
        var bAngle = Math.Atan(ins.B.Y / (double)ins.B.X);
    ...
    private (int Y, int X) ParseFixedPrize(string input)
    {
